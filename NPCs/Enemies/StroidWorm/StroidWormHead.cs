...
namespace Archeon.NPCs.Enemies.StroidWorm
{
	...
	public class StroidWormHead : ModNPC
	{
		...
		public override void SetDefaults()
		{
			...
			base.npc.value = (float)Item.buyPrice(0, 0, 40, 75);
		...
		public override bool PreAI()
		{
			if (Main.netMode != 1 && base.npc.ai[0] == 0f)
			{
				...
				int num2 = Main.rand.Next(6, 7);
				for (int i = 0; i < num2; i++)
				{
					num = NPC.NewNPC((int)base.npc.Center.X, (int)base.npc.Center.Y, base.mod.NPCType("StroidWormBody"), base.npc.whoAmI, 0f, (float)num, 0f, 0f, 255);
					Main.npc[num].realLife = base.npc.whoAmI;
					Main.npc[num].ai[3] = (float)base.npc.whoAmI;
				}
				num = NPC.NewNPC((int)base.npc.Center.X, (int)base.npc.Center.Y, base.mod.NPCType("StroidWormTail"), base.npc.whoAmI, 0f, (float)num, 0f, 0f, 255);
				Main.npc[num].realLife = base.npc.whoAmI;
				Main.npc[num].ai[3] = (float)base.npc.whoAmI;
				base.npc.ai[0] = 1f;
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			if (base.npc.life <= 0)
			{
				for (int i = 0; i < 7; i++)
				{
					...
					dust.scale *= 1.6f + (float)Main.rand.Next(-31, 31) * 0.01f;
				}
				Gore.NewGore(base.npc.position, base.npc.velocity, base.mod.GetGoreSlot("Gores/StroidWormHeadGore"), 1f);
			}
		}
		...
		public override void NPCLoot()
		{
			if (Main.expertMode)
			{
				...
				{
					Item.NewItem((int)base.npc.position.X, (int)base.npc.position.Y, base.npc.width, base.npc.height, base.mod.ItemType("StroidOre"), Main.rand.Next(12, 24), false, 0, false, false);
					return;
				}
			}
			if (!Main.expertMode)
			{
				if (Main.rand.Next(6) == 0)
				{
					Item.NewItem((int)base.npc.position.X, (int)base.npc.position.Y, base.npc.width, base.npc.height, base.mod.ItemType("StroidOre"), Main.rand.Next(8, 16), false, 0, false, false);
				}
			}
		}
		...
		public override float SpawnChance(NPCSpawnInfo spawnInfo)
		{
			if (Main.dayTime)
			{
				if (spawnInfo.player.GetModPlayer<MyPlayer>().zoneStroidBiome)
				{
					return 1.7f;
				}
				return 0f;
