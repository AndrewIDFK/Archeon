...
namespace Archeon.Items.Projectiles.GemProj
{
	...
	public class SapphireWeb2 : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (base.projectile.localAI[0] > 3f)
			{
				for (int i = 0; i < 8; i++)
				{
					...
					Main.dust[num].scale = (float)Main.rand.Next(58, 74) * 0.0289f;
		...
		public override void Kill(int Timeleft)
		{
			...
			for (int i = 0; i < numberProjectiles; i++)
			{
				...
				value *= scaleFactor;
				Projectile.NewProjectile(base.projectile.Center.X, base.projectile.Center.Y - 5, value.X, value.Y, base.mod.ProjectileType("SapphireWeb3"), base.projectile.damage, base.projectile.knockBack, Main.myPlayer, 0f, 0f);
			}
			base.projectile.netUpdate = true;
			Main.PlaySound(2, (int)base.projectile.position.X, (int)base.projectile.position.Y, 12, 1f, 0f);
			base.projectile.ai[0] = -50f;
		
			Projectile.NewProjectile(base.projectile.Center.X - 4.5f, base.projectile.Center.Y + 4.5f, projectile.velocity.X, projectile.velocity.Y - 8.21f, base.mod.ProjectileType("SapphireWebSplit"), base.projectile.damage, base.projectile.knockBack, Main.myPlayer, 0f, 0f);
