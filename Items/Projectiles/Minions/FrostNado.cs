...
namespace Archeon.Items.Projectiles.Minions
{
	...
	public class FrostNado : ModProjectile
	{
		// Token: 0x06000DFD RID: 3581 RVA: 0x000ADB0C File Offset: 0x000ABD0C
		public override void SetStaticDefaults()
		{
			base.DisplayName.SetDefault("Frostnado");
		...
		public override void AI()
		{
			if (base.projectile.localAI[1] == 0f)
			{
				...
				for (int i = 0; i < num; i++)
				{
					...
					int num2 = Dust.NewDust(value + vector, 0, 0, base.mod.DustType("FrostElementDust"), vector.X * 1.5f, vector.Y * 1.5f, 100, default(Color), 1.4f);
			...
			bool flag = base.projectile.type == base.mod.ProjectileType("FrostNado");
			...
			if (flag2 && base.projectile.ai[0] == 0f)
			{
				...
				{
					base.projectile.velocity *= 0.76f;
				}
				if (num9 > 150f)
				{
			...
			else
			{
				...
				else
				{
					...
					base.projectile.velocity *= 1.09f;
				}
			}
			...
			{
				int num13 = Dust.NewDust(new Vector2(base.projectile.position.X, base.projectile.position.Y), base.projectile.width, base.projectile.height, base.mod.DustType("FrostElementDust"), 0f, 0f, 100, default(Color), 2f);
			...
			if (base.projectile.ai[0] == 0f)
			{
				...
				int type = base.mod.ProjectileType("FrostNadoShot");
				if (flag2 && !Collision.SolidCollision(base.projectile.position, base.projectile.width, base.projectile.height) && base.projectile.ai[1] == 0f)
				{
					...
					if (Main.myPlayer == base.projectile.owner)
					{
						...
						int num14 = Projectile.NewProjectile(base.projectile.Center.X, base.projectile.Center.Y, vector7.X - 3, vector7.Y - 3, type, base.projectile.damage - 13, 0f, Main.myPlayer, 0f, 0f);
						Main.projectile[num14].timeLeft = 480;
