...
namespace Archeon.Items.Projectiles.Minions
{
	...
	public class FireNado : ModProjectile
	{
		...
		{
			base.DisplayName.SetDefault("Firenado");
		...
		public override void AI()
		{
			if (base.projectile.localAI[1] == 0f)
			{
				...
				for (int i = 0; i < num; i++)
				{
					...
					int num2 = Dust.NewDust(value + vector, 0, 0, base.mod.DustType("FireElementDust"), vector.X * 1.5f, vector.Y * 1.5f, 100, default(Color), 1.4f);
			...
			bool flag = base.projectile.type == base.mod.ProjectileType("FireNado");
			Player player = Main.player[base.projectile.owner];
			MyPlayer modPlayer = player.GetModPlayer<MyPlayer>();
			player.AddBuff(base.mod.BuffType("FireNadoBuff"), 3600, true);
			if (flag)
			{
				if (player.dead)
				{
			...
			if (flag2 && base.projectile.ai[0] == 0f)
			{
				...
				{
					base.projectile.velocity *= 1.08f;
				}
				if (num9 > 200f)
				{
			...
			else
			{
				...
				else
				{
					...
					base.projectile.velocity *= 0.85f;
				}
			}
			...
			{
				int num13 = Dust.NewDust(new Vector2(base.projectile.position.X, base.projectile.position.Y), base.projectile.width, base.projectile.height, base.mod.DustType("FireElementDust"), 0f, 0f, 100, default(Color), 2f);
			...
			if (base.projectile.ai[0] == 0f)
			{
				...
				int type = base.mod.ProjectileType("FireNadoShot");
				if (flag2 && !Collision.SolidCollision(base.projectile.position, base.projectile.width, base.projectile.height) && base.projectile.ai[1] == 0f)
				{
					...
					if (Main.myPlayer == base.projectile.owner)
					{
						...
						int num14 = Projectile.NewProjectile(base.projectile.Center.X, base.projectile.Center.Y, vector7.X + 3, vector7.Y + 3, type, base.projectile.damage, 0f, Main.myPlayer, 0f, 0f);
						Main.projectile[num14].timeLeft = 380;
