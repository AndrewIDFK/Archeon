		...
		public override void AI()
		{
			if (base.projectile.localAI[0] == 0f)
			{
				...
				for (int i = 0; i < num; i++)
				{
					...
					int num2 = Dust.NewDust(value + vector, 0, 0, base.mod.DustType("AirElementDust"), vector.X * 1.5f, vector.Y * 1.5f, 100, default(Color), 1.4f);
			...
			int num3 = Dust.NewDust(base.projectile.position, base.projectile.width, base.projectile.height, base.mod.DustType("AirElementDust"), 0f, 0f, 0, default(Color), 1f);
			Main.dust[num3].velocity *= 0.1f;
			Main.dust[num3].scale = 1.4f;
			Main.dust[num3].noGravity = true;
			
			bool flag = base.projectile.type == base.mod.ProjectileType("AirNado");
			...
			{
				if (j != base.projectile.whoAmI && Main.projectile[j].active && Main.projectile[j].owner == base.projectile.owner && Main.projectile[j].type == base.mod.ProjectileType("AirNado") && Math.Abs(base.projectile.position.X - Main.projectile[j].position.X) + Math.Abs(base.projectile.position.Y - Main.projectile[j].position.Y) < (float)base.projectile.width)
				{
			...
			if (!flag2)
			{
				...
				float num17 = Main.player[base.projectile.owner].Center.Y - vector2.Y - 40f;
				...
				else
				{
					...
					base.projectile.velocity *= 1.13f;
				}
				base.projectile.rotation = base.projectile.velocity.X * 0.05f;
			...
			else
			{
				...
				if (base.projectile.ai[1] == 0f)
				{
					...
					num20 *= num22;
					num21 *= num22;
					base.projectile.velocity.X = (base.projectile.velocity.X * 14f + num20) / 14f;
					base.projectile.velocity.Y = (base.projectile.velocity.Y * 14f + num21) / 14f;
				}
				...
				base.projectile.rotation = base.projectile.velocity.X * 0.05f;
				if ((double)Math.Abs(base.projectile.velocity.X) > 0.3)
				{
					base.projectile.spriteDirection = -base.projectile.direction;
					return;
				}
